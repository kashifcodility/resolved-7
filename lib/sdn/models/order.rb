class Order
    ...
    const_def :FREEZE_DELIVERY_TIMEOFDAY, { hour: 16, min: 30 } # 4:30PM for delivery
    const_def :FREEZE_DELIVERY_DAYS,      2 # 2 days prior on every non-Monday day for delivery
    const_def :FREEZE_PICKUP_TIMEOFDAY,   { hour: 16, min: 30 } # 4:30PM for pickup
    ...
    def rental?()                type == 'Rental'    end
    def sales?()                 type == 'Sales'     end
    def self.rentals()           all(type: 'Rental') end
    def self.sales()             all(type: 'Sales')  end
    def self.renting_rentals()   all(type: 'Rental', status: 'Renting')  end # TODO: Replace useage of this with .renting.rentals
    ...
    def next_rental_renewal
        return nil unless type == 'Rental' && status.among?('Open','Pulled','InTransit','Renting')
    ...
    def total
        order_lines.not_voided.pluck(:price).sum
    end
    ...
    def sibling_rental_orders(only_currently_renting: true, include_source: false)
        query = %{
            ...
            AND orders.id != #{id}
            AND orders.type IN ('Rental', 'Sales')
    ...
    def self.orders_for_user(user_id, status='all')
        $DB.select <<-FOO
            SELECT o.id, o.type, o.status, o.changed, ordered_date, due_date, ppl.created AS created, COUNT(l.product_id) AS order_lines,
    ...
    def self.rental_order_details(user, order, status: 'NULL', sort_by: 'o.ordered_date DESC')
        ...
        results = $DB.select <<-FOO
            SELECT p.id, p.product, l.image as main_image,l.width AS main_image_width, l.height AS main_image_height, p.width, p.height, p.depth,
                ...
                p.quantity,DATE_FORMAT(loc.created,'%b %e %Y') AS last_log_date, o.type AS order_type,
