class Barcode
    ...
    def self.dimensions_minmax_range(filters: nil, pids: [], intent: nil, round: true)
        ...
        dims = Product.all(fields: [ :width, :depth, :height ], id: ids)
    ...
    def self.query_available_product_ids(**o)
        # Default values
        o[:site_id]                   = o[:site_id].to_i if o[:site_id]
        ...
        w = "TRUE "
        group = "`barcodes`.`product_id` "
        h = "TRUE "

        p = []        
        
        if o[:count_only]
            s = "COUNT(DISTINCT `barcodes`.`product_id`) "
            group = "NULL"
        else
            s = "`barcodes`.`product_id` "
        end
        ...
        if o[:site_id]
            h += "AND MIN(`barcodes`.`site_id`) = #{o[:site_id]} "
        end
        
        if (o.keys & [ :category_id, :excluded_category_ids, :exclude_ids_category, :premium, :closeout, :discount_rental, :on_sale, :color_id, :material_id, :search_product, :search_category, :bed_size_id, :sofa_size_id, ]).any?
            ...
            if o[:category_id] > 0
                category_ids = Category.get(o[:category_id])&.id_tree || [ 99999 ] # causes no results if category doesn't exist
                w += "AND `yuxi_products`.`category_id` IN (%s) " % [ category_ids.join(',') ]
            end

            yp_bool_sql = ->(db_col, key) do
                yn = o.fetch(key, nil)
                w += "AND `yuxi_products`.`%s` = %s " % [ db_col, yn == 'y' ? 'TRUE' : 'FALSE' ] if yn.among?('y', 'n')
            end
            ...
            if o[:color_id]
                w += "AND `yuxi_products`.`color_id` = #{o[:color_id].to_i} "
            end
        ...
        if o[:search_query]
            w += "AND (`products`.`product` LIKE ? OR `products`.`id` = ?) "
            p << "%#{o[:search_query]}%"
        ...
        query = "SELECT #{s} FROM `barcodes` #{j} WHERE (#{w}) GROUP BY #{group} HAVING #{h} ORDER BY #{order} LIMIT #{o[:offset]}, #{o[:limit]}"
        
        return repository(:default).adapter.select(query, *p) unless p.empty?
